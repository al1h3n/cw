...
| D23 | **Exam/broadcast lockdown uses Windows *policies*, not a kernel driver.** Blocking Win+L and stripping the Ctrl+Alt+Del screen is done by toggling documented registry policies (`DisableLockWorkstation`, `DisableTaskMgr`, `NoLogoff`, `DisableChangePassword`, `HideFastUserSwitching` under `HKLM\…\Policies\System`) at lock start and reverting them at lock end, alongside the user-space keyboard hook (`platform::keyguard`) and the separate-desktop lock. **No kernel-mode keyboard-filter driver** — a signed driver costs money and needs the WDK/signing the project cannot fund yet. | User choice ("without signing anything — I'm broke now"). Policies need no signing, are reversible, and block Win+L, which a user-space hook cannot. The SAS (Ctrl+Alt+Del) still fires, but with its entries removed there is nothing actionable on it. A driver/kiosk route stays open for later. |
